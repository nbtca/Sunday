<template>
  <div class="flex w-full justify-between sm:(bg-base-self flex-col h-full border-r p-2)">
    <div class="flex flex-col-reverse w-full relative sm:block">
      <div class="flex h-15 p-2 z-40 justify-between items-center md:(p-0 h-24)">
        <div
          class="font-bold font-mono text-center tracking-wide text-3xl italic text-shadow-sm uppercase sm:(w-full) lg:text-4xl"
          @click="
            $router.push('/');
            isOpen = false;
          "
        ...
        <!-- <button class="h-11 w-11 flex items-center justify-center rounded-xl">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 115.99 114.94" class="h-8 fill-current text-gray-900">
            <g id="Regular-M">
              <path
      ...
      >
        <div class="">
          <div
            class="p-1 divide-gray-400/20 rounded-xl backdrop-filter backdrop-blur-md shadow-sm overflow-hidden border border-gray-400/30"
          >
            <button
    ...
    <div class="flex-col mb-4 hidden items-center sm:(flex)">
      ...
      <div class="relative">
        ...
        <span v-if="role == 'admin'" class="bg-green-100 h-5 -top-2 -right-6 bottom-0 text-green-800 badge"> 管理员 </span>
      </div>
      <!-- <div>event count</div> -->
    ...
    <bottom-dialog ref="BottomDialog">
      <template #body>
        <form class=""></form>
        <div class="cellsm materialThin border-none bg-opacity-80">
          <div class="flex">
            <div class="rounded-full h-17 w-17 overflow-hidden block">
              <img class="" :src="accountInfo.ravatar" alt="" />
            </div>
            <button class="textLink">上传</button>
          </div>
          <div>
            <div>{{accountInfo.rid}}</div>
            <div>{{accountInfo.gmt_create}}</div>
          </div>
        </div>
        <input-section subject="昵称">
          <input-base subject="" :val="accountInfo.ralias"></input-base>
        </input-section>
